...
if($Checken) {	
	foreach($Termen as $term) {
		...
		$url			= getURL($term);		
		$even			= true;
		$URL			= "http://www.marktplaats.nl/index.php?url=". urlencode($url);		
		$inhoud		= file_get_contents($url);
				
		//echo "RSS : $rss|";
		
		if(strpos($inhoud, '<td class="admarkt_info_')) {
			$beginString = '<td class="admarkt_info_';
		} else {
			$beginString = '';
		}
		...
		if($eindString == '' AND strpos($inhoud, '<td class="admarkt_bottom_list_header" colspan="6">')) {
			$eindString = '<td class="admarkt_bottom_list_header" colspan="6">';
		}
		
		$string	= getString($beginString, $eindString, $inhoud, 0);
		
		$array		= explode('<tr id="AD', $string[0]);
		$aantal		= count($array);
		...
		for($i=1 ; $i <= $maximum ; $i++) {			
			...
			if(!strpos($array[$i], 'Heel Nederland') AND !$SPAM) {							
				...
				if($array[$i] != "") {
					$data			= getMarktplaatsData_v2($array[$i]);
					...
					if(NewItem($data, $term) OR changedItem($data, $term) OR $debug == 1) {					
						if($rss == 0 OR $rss == 2) {
							...
							$Item .= "</tr>\n";
							$Item .= "<tr>\n";
							$Item .= "	<td align='right'><b>". $data['price'] ."</b>". ($data['price_add'] != '' ? ' <small>('. $data['price_add'] .')</small>' : '') ." | <i><a href='http://verkopers.marktplaats.nl/". $data['verkoper_id'] ."'>". $data['verkoper'] ."</a></i> | <a href='http://maps.google.nl/maps?q=". $data['postcode'] ."%2C+Nederland&z=9'>". $data['plaats'] ."</a> (". $data['afstand'] ." km) | ". strftime("%a %e %b %H:%M", $data['date']) ."</td>\n";
							$Item .= "</tr>\n";
							$Item .= "<tr>\n";
							$Item .= "	<td>". makeTextBlock($data['descr_long'], 500) ."</td>\n";
							$Item .= "</tr>\n";
							$Item .= "<tr>\n";
							$Item .= "	<td><img src='". implode("'>&nbsp;<img src='", explode("|", $data['picture'])). "'></td>\n";
							$Item .= "</tr>\n";
							$Item .= "</table>\n";
